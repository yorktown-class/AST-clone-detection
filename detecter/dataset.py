...
class PairCodeset(data.Dataset):
    ...
    def drop(self, max_node_count: int):
        ...
        def check_length(series):
            return (
                tree_tools.peep_tree_nodes(self.code_data.loc[series["lhs"], "tree"]) <= max_node_count
                and tree_tools.peep_tree_nodes(self.code_data.loc[series["rhs"], "tree"]) <= max_node_count
            )
